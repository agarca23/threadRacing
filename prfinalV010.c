...
void *accionesCorredor(void *numCorredor){
	...
	while(1){
		if(corredores[*(int*)numCorredor].id!=0 && corredores[*(int*)numCorredor].correr==0){
			pthread_mutex_lock(&semaforoCorredor);
			writeLog(id, msg5, identificadorCorredor, 0);
			corredores[*(int*)numCorredor].id=0;
                        corredores[*(int*)numCorredor].estado=1;
			corredores[*(int*)numCorredor].correr=1;
			corredores[*(int*)numCorredor].sancionado=0;
			corredores[*(int*)numCorredor].tiempoTotal=0;
			corredores[*(int*)numCorredor].terminado=0;
			pthread_mutex_unlock(&semaforoCorredor);
			pthread_exit(NULL);
		}
		while(numVuelta!=20 && corredores[*(int*)numCorredor].correr==1){
			pthread_mutex_lock(&semaforoCorredor);
			...
			if(problemas==1){
				...
				if(problemasGraves>7){
					sleep(1);
					pthread_mutex_lock(&semaforoCorredor);
					corredores[*(int*)numCorredor].correr=0;
					pthread_mutex_unlock(&semaforoCorredor);
				}
			}
